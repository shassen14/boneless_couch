# Deploying on Raspberry Pi (Production)
...
**2. Run migrations** (first time, or after pulling changes that add migrations)
...
After setting `BOT_LOGS_WEBHOOK_URL` and/or `SENTRY_DSN` in `.env`, confirm they're wired up:
