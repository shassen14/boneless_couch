...
from couchd.core.models import GuildConfig, ProblemPost, StreamEvent, ProblemAttempt, SolutionPost
...
class ProblemsWatcherCog(commands.Cog):
    ...
    async def _seed_watermark(self):
        ...
        log.info("ProblemsWatcherCog watermark seeded at attempt_id=%d", self.last_processed_attempt_id)
    ...
    async def _build_embed(self, slug: str):
        ...
        embed.add_field(name=f"Appearances ({len(attempts)})", value=appearances, inline=False)

        async with get_session() as db:
            sol_count = (await db.execute(
